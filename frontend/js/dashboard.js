  ...
  paginatedStudents.forEach((student) => {
    const attendance =
      ...
      student.attendance_percentage !== undefined
        ? `${formatOneDecimal(student.attendance_percentage)}%`
        : "N/A";

    const assessmentAvg =
      student.assessment_average !== null &&
      student.assessment_average !== undefined
        ? formatOneDecimal(student.assessment_average)
        : "N/A";

    const predictedScore =
      student.predicted_score !== null &&
      student.predicted_score !== undefined
        ? formatOneDecimal(student.predicted_score)
        : "0.0";
    ...
    html += `
      <tr class="hover:bg-slate-50 transition-colors">
        ...
        <td class="px-5 py-4 text-slate-700">${attendance}</td>
        <td class="px-5 py-4 text-slate-700">${assessmentAvg}</td>
        <td class="px-5 py-4 font-semibold text-primary">${predictedScore}</td>
        <td class="px-5 py-4">
...
function exportToCSV() {
  ...
  const rows = filteredStudents.map((student) => [
    ...
    student.attendance_percentage !== null && student.attendance_percentage !== undefined
      ? formatOneDecimal(student.attendance_percentage)
      : "N/A",
    student.assessment_average !== null && student.assessment_average !== undefined
      ? formatOneDecimal(student.assessment_average)
      : "N/A",
    student.predicted_score !== null && student.predicted_score !== undefined
      ? formatOneDecimal(student.predicted_score)
      : "0.0",
...
async function debugStudents() {
  const output = document.getElementById("debugOutput");
  if (!output) return;
  output.innerHTML = `<pre>${escapeHtml(JSON.stringify(allStudents, null, 2))}</pre>`;
}
...
async function testPredictionGeneration() {
  ...
  try {
    ...
    const result = await apiCall(`/prediction/generate/${students[0].id}`, "POST");
